...
%inputs:
lc = [ 0 0 0 0 0 ; 0 -dc1 0 1 0 ; 0 0 -dc2 0 0 ; 0 -dc3 0 0 0 ; 0 x03-xc3 0 1 2 ];
m = [ m1 , m2 , m3 ];
I = sym ( zeros ( 3 , 3 , 3 ) );
...
j(1,:,2) = [ 0 0 0 -d2 0 ];
j(1,:,3) = [ 2 inf -d3 0 0 ];
j(1:2,:,4) = [ 3 inf 0 0 -d4 ; 1 inf 0 0 0 ];
% jkd = sym (zeros ( 3 , 2 , 9 ) );
jkd = sym (zeros ( 3 , 2 , 4 ) );
% jkd(2,:,4) = [ cv11 0 ]; jkd(2,:,5) = [ cv12 0 ]; jkd(2,:,6) = [ cv13 0 ];
% jkd(1:2,:,7) = [ k2 0; cv2 0 ]; jkd(1:2,:,8) = [ k31 0; cv31 0 ]; jkd(1:2,:,9) = [ k32 0; cv32 0 ];
jkd(1,:,1) = [ F1 0 ]; jkd(1:2,:,2) = [ k2 0; cv2 0 ]; jkd(1:2,:,3) = [ k31 0; cv31 0 ]; jkd(1:2,:,4) = [ k32 0; cv32 0 ];
